    ...
    public static TablePointData read(File file, int sheetIndex) throws Exception {
        ...
        try (FileInputStream fis = new FileInputStream(file);
            ...
            if (sheetIndex < 0 || sheetIndex >= workbook.getNumberOfSheets()) {
                throw new RuntimeException("Índice de hoja XLSX inválido.");
            }

            XSSFSheet sheet = workbook.getSheetAt(sheetIndex);
            if (sheet == null) {
                throw new RuntimeException("No se encontró la hoja seleccionada en el archivo XLSX.");
            }

            Row headerRow = sheet.getRow(sheet.getFirstRowNum());
            if (headerRow == null) {
                throw new RuntimeException("La hoja XLSX está vacía.");
            }
