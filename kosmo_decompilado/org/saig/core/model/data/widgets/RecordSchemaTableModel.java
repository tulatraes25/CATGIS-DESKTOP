...
public class RecordSchemaTableModel
extends ColumnBasedTableModel {
    private static final Logger LOGGER = Logger.getLogger(RecordSchemaTableModel.class);
    private Table table;
    ...
    public RecordSchemaTableModel(Table table) {
        this.table = table;
        this.initialize();
    ...
    public synchronized Object getValueAt2(int row, int col) {
        int i;
        ...
        List<Record> features = this.table.getByPrimaryKeys(keys_.toArray());
    ...
    public synchronized Object getRecordValueAt(int row) {
        int i;
        Object key = this.keys.get(row);
        Object value = this.cache.get(key);
        if (value != null) {
    ...
    public void removeAll(Collection recordsToRemove) {
        for (Feature feature : recordsToRemove) {
            int row = -1;
    ...
    public void moveRowUp(Object value) {
        int selectedRow = this.keys.indexOf(value);
        if (selectedRow == 0) {
            return;
        }
        Object previousObject = this.keys.get(selectedRow - 1);
        this.keys.set(selectedRow, previousObject);
        this.keys.set(selectedRow - 1, value);
    }

    public void moveRowDown(Object value) {
        int selectedRow = this.keys.indexOf(value);
        if (selectedRow == this.keys.size() - 1) {
            return;
        }
        Object nextObject = this.keys.get(selectedRow + 1);
        this.keys.set(selectedRow, nextObject);
        this.keys.set(selectedRow + 1, value);
    }
    ...
    public void updateAll(Collection features, Collection oldFeatureClones) {
    ...
    public List<Record> getSelectedRecords(int[] rows) {
        ArrayList<Record> selectedRecords = new ArrayList<Record>();
        if (rows != null) {
            int i = 0;
            while (i < rows.length) {
                selectedRecords.add((Record)this.getRecordValueAt(rows[i]));
                ++i;
            }
        }
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        ...
        Record record = (Record)column.getValueAt(rowIndex);
        if (record != null) {
            return record.getAttribute(column.getAttribute().getName());
        }
        return null;
    }
    ...
    extends ColumnBasedTableModel.Column {
        ...
        public Object getValueAt(int rowIndex) {
            return RecordSchemaTableModel.this.getRecordValueAt(rowIndex);
        }
