...
implements IRenderer {
    ...
    @Override
    public void render(Image image, Envelope viewEnvelope, Layerable layerable, double angle, double panelScale, boolean strategy, Unit<Length> mapUnits, Map<Object, Object> renderingHints) {
        block17: {
            this.layer = (Layer)layerable;
            this.envelope = viewEnvelope;
            this.angle = angle;
            ...
            try {
                try {
                    ...
                    block16: {
                        ...
                        Envelope envelope = this.getEnvelopeInModelCoordinatesForQuery();
                        ...
                        Object[] rules = this.getRulesInScale(fts.getRules(), scaleRange, panelScale, envelope.getWidth() / (double)this.width, mapUnits, this.factor);
                    ...
                    Rectangle paintArea = new Rectangle(0, 0, this.width, this.height);
                    Graphics2D g = this.g2d;
                    labelCache.endLayer(this.layer.getName(), g, paintArea);
    ...
    private void paintAll(ShapeFileDataSource ds, boolean contains, List<RuleStyle> rulesWithFilter, AffineTransform affineTransform, RuleStyle elseRule, LabelCache labelCache, MathTransform2D transform, NumberRange scaleRange, boolean hasFeature, boolean loadFeatureGeometry, Rectangle2D extent, String layerId) throws Exception {
        ...
        if (!ds.hasEditableFeatures()) {
            if (ds.getShapeType() == 1 || ds.getShapeType() == 11) {
                int i = 0;
                while (i < ds.iterableRows() && !this.cancelled) {
                ...
                int i = 0;
                while (i < ds.getShapeCount() && !this.cancelled) {
                    Rectangle2D bounds = this.con.getShapeBounds(i);
                    if (bounds != null) {
                        if (this.ct != null) {
                            bounds = this.ct.convert(bounds);
                        ...
                        if (contains || XRectangle2D.intersectInclusive((Rectangle2D)extent, (Rectangle2D)bounds)) {
                            if (bounds.getWidth() > distance || bounds.getHeight() > distance) {
                                ...
                                this.process1Px(this.g2d, bounds, affineTransform, rulesWithFilter, elseRule, hasFeature, loadFeatureGeometry, i, ds);
                            }
                        }
                    }
                    ++i;
    ...
    private void paintAllWithSpatialIndex(ShapeFileDataSource ds, List<RuleStyle> rulesWithFilter, AffineTransform affineTransform, RuleStyle elseRule, LabelCache labelCache, MathTransform2D transform, NumberRange scaleRange, boolean hasFeature, boolean loadFeatureGeometry, Rectangle2D extent, String layerId) throws Exception {
        ...
        if (ds.getShapeType() == 1 || ds.getShapeType() == 11) {
            ...
            while (iter.hasNext() && !this.cancelled) {
                ...
                this.process1Px(this.g2d, bounds, affineTransform, rulesWithFilter, elseRule, hasFeature, loadFeatureGeometry, index, ds);
            }
        }
    }
...
lbl-1000:
...

            {
                ...
                this.process1Px(this.g2d, bounds, affineTransform, rulesWithFilter, elseRule, feat);
lbl35:
                // 4 sources
    ...
    private void processPoint(String layerId, Feature feat, List<RuleStyle> rulesWithFilter, AffineTransform affineTransform, RuleStyle elseRule, LabelCache labelCache, MathTransform2D transform, NumberRange scaleRange, IShapeGeometry geom) throws Exception {
        if (this.ct != null) {
            geom = geom.cloneGeometry();
            geom.reProject(this.ct);
        }
        IShapeGeometry geom2 = geom;
        boolean check = false;
        if (elseRule != null && elseRule.getFilter().contains(feat)) {
            LiteShape2 shape = this.drawShape(this.g2d, geom2.cloneGeometry(), elseRule, labelCache, transform, scaleRange, feat, this.ct, this.isLineLayer);
            this.processTextSymbolizer(layerId, elseRule, shape, labelCache, feat, this.layerWidth, this.envelope.getWidth(), scaleRange, transform, this.ct);
            check = true;
        }
        ...
        while (iter.hasNext() && !check) {
            ...
            LiteShape2 shape = this.drawShape(this.g2d, geom2.cloneGeometry(), ruleStyle, labelCache, transform, scaleRange, feat, this.ct, this.isLineLayer);
            this.processTextSymbolizer(layerId, ruleStyle, shape, labelCache, feat, this.layerWidth, this.envelope.getWidth(), scaleRange, transform, this.ct);
        }
    }
    ...
    private void processPoint(ShapeFileDataSource ds, int index, List<RuleStyle> rulesWithFilter, AffineTransform affineTransform, RuleStyle elseRule, LabelCache labelCache, MathTransform2D transform, NumberRange scaleRange, boolean hasFeature, boolean loadFeature, ShapeGeometry geom, String layerId) throws Exception {
        if (geom == null) {
        ...
        ShapeGeometry geom2 = geom;
        boolean check = false;
        if (elseRule != null && elseRule.getFilter().contains(feature)) {
            LiteShape2 shape = this.drawShape(this.g2d, geom2.cloneGeometry(), elseRule, labelCache, transform, scaleRange, feat2, this.ct, this.isLineLayer);
            this.processTextSymbolizer(layerId, elseRule, shape, labelCache, feat2, this.layerWidth, this.envelope.getWidth(), scaleRange, transform, this.ct);
            check = true;
        }
        Iterator<RuleStyle> iter = rulesWithFilter.iterator();
        while (iter.hasNext() && !check) {
            ...
            LiteShape2 shape = this.drawShape(this.g2d, geom2.cloneGeometry(), ruleStyle, labelCache, transform, scaleRange, feat2, this.ct, this.isLineLayer);
            this.processTextSymbolizer(layerId, ruleStyle, shape, labelCache, feat2, this.layerWidth, this.envelope.getWidth(), scaleRange, transform, this.ct);
        }
    }

    private void processShape(ShapeFileDataSource ds, int index, List<RuleStyle> rulesWithFilter, AffineTransform affineTransform, RuleStyle elseRule, LabelCache labelCache, MathTransform2D transform, NumberRange scaleRange, boolean hasFeature, boolean loadFeature, String layerId) throws Exception {
        boolean check = false;
        ShapeGeometry geom = this.con.getShape(index);
        Feature feature = null;
        if (hasFeature) {
    ...
    private void processShape(String layerId, Feature feat, List<RuleStyle> rulesWithFilter, AffineTransform affineTransform, RuleStyle elseRule, LabelCache labelCache, MathTransform2D transform, NumberRange scaleRange, IShapeGeometry geom) throws Exception {
        boolean check = false;
        if (this.ct != null) {
            geom = geom.cloneGeometry();
            geom.reProject(this.ct);
    ...
    protected Object[] getRulesInScale(Rule[] rules, NumberRange scaleRange, double panelScale, double pixelSize, Unit<Length> mapUnits, double factor) {
        ...
        while (i < rules.length) {
            ...
            if (rule.isEnabled()) {
                ...
                if ((rule.getMinScaleDenominator() <= panelScale || Double.isNaN(rule.getMinScaleDenominator())) && (rule.getMaxScaleDenominator() >= panelScale || Double.isNaN(rule.getMaxScaleDenominator()))) {
                    RuleStyle ruleStyle = new RuleStyle(rule, scaleRange, false, factor, pixelSize, mapUnits, this.layer.getFeatureSchema());
    ...
    private void draw1px(Rectangle2D bounds, AffineTransform aft, int rgb) {
        ...
        Point2D pDest2 = this.g2d.getTransform().transform(pDest, null);
