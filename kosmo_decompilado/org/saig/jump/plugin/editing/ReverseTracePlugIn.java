...
public class ReverseTracePlugIn
extends AbstractPlugIn {
    public static final String NAME = I18N.getString("org.saig.jump.plugin.editing.ReverseTracePlugIn.Reverse-trace");
    public static final Icon ICON = GUIUtil.resize(IconLoader.icon("reverse_trace.png"), 20);

    @Override
    public boolean execute(PlugInContext context) {
        ReverseTracePlugIn.getInProgressCursorTool().reverseCoordinates();
        return true;
    }
    ...
    public EnableCheck getCheck() {
        return ReverseTracePlugIn.createEnableCheck(JUMPWorkbench.getFrameInstance().getContext());
    }
    ...
    public static EnableCheck createEnableCheck(WorkbenchContext workbenchContext) {
        EnableCheck check = new EnableCheck(){
            ...
            public String check(JComponent component) {
                boolean cond = false;
                boolean bl = cond = ReverseTracePlugIn.getInProgressCursorTool() != null;
                if (!cond) {
                    return "";
                }
                return null;
