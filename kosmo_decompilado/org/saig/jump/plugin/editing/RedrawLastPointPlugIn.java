...
public class RedrawLastPointPlugIn
extends AbstractPlugIn {
    public static final String NAME = I18N.getString("org.saig.jump.plugin.editing.RedrawLastPointPlugIn.Redraw-last-removed-point");
    public static final Icon ICON = GUIUtil.resize(IconLoader.icon("redo_trace.png"), 20);

    @Override
    public boolean execute(PlugInContext context) {
        RedrawLastPointPlugIn.getInProgressCursorTool().redoLastCoordinate();
        return true;
    }
    ...
    public EnableCheck getCheck() {
        return RedrawLastPointPlugIn.createEnableCheck(JUMPWorkbench.getFrameInstance().getContext());
    }
    ...
    public static EnableCheck createEnableCheck(WorkbenchContext workbenchContext) {
        EnableCheck check = new EnableCheck(){
            ...
            public String check(JComponent component) {
                boolean cond = false;
                boolean bl = cond = RedrawLastPointPlugIn.getInProgressCursorTool() != null;
                if (!cond) {
                    return "";
                }
                return null;
