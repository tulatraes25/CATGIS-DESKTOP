...
public class XYZCoordinatesVertexMarkerDecorator
extends VertexMarkerDecorator {
    public static final String NAME = I18N.getString("es.kosmo.core.renderer.decorators.impl.XYZCoordinatesVertexMarkerDecorator.Vertex-X-Y-Z-Coordinates");
    public static final Icon ICON = IconLoader.icon("VertexXYDecorator.gif");

    public XYZCoordinatesVertexMarkerDecorator() {
        this(Color.BLACK, DEFAULT_FONT, "pixel");
    }

    public XYZCoordinatesVertexMarkerDecorator(Color color, Font font, String units) {
        this.name = NAME;
        this.icon = ICON;
        this.setColor(color);
        this.setFont(font);
    ...
    protected void paintGeometry(Geometry geometry, Graphics2D graphics, IDecoratorPoint2DTransformer viewport, double pixelSize, Unit<Length> viewUnit) throws Exception {
        ...
        while (i < coords.length) {
            ...
            String label = "X = " + NumberFormatManager.getFormattedValue(currentCoord.x) + "; Y = " + NumberFormatManager.getFormattedValue(currentCoord.y);
            if (!Double.isNaN(currentCoord.z)) {
                label = String.valueOf(label) + "; Z = " + NumberFormatManager.getFormattedValue(currentCoord.z);
            }
            this.setText(label);
