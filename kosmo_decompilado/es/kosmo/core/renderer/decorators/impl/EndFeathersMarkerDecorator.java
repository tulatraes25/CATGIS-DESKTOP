...
import es.kosmo.core.renderer.decorators.impl.FeathersMarkerDecorator;
...
public class EndFeathersMarkerDecorator
extends FeathersMarkerDecorator {
    public static final String NAME = I18N.getString("es.kosmo.core.renderer.decorators.impl.EndFeathersMarkerDecorator.Ending-feathers");
    public static final Icon ICON = IconLoader.icon("FeathersEnd.gif");

    public EndFeathersMarkerDecorator() {
        this(Color.BLACK, 10.0, 30.0, false, 0.0, "pixel");
    }

    public EndFeathersMarkerDecorator(Color color, double size, double sharpness, boolean isFixedRotation, double rotation, String units) {
        this.name = NAME;
        this.icon = ICON;
        this.reverse = true;
    ...
    protected void paintGeometry(Geometry geometry, Graphics2D graphics, IDecoratorPoint2DTransformer viewport, double pixelSize, Unit<Length> viewUnit) throws Exception {
        ...
        while (i < geometry.getNumGeometries()) {
            ...
            Coordinate coord = currentGeom.getCoordinates()[currentGeom.getCoordinates().length - 1];
            ...
            if (!this.isFixedRotation()) {
                Coordinate previousCoord = this.getPreviousNonEqual(currentGeom);
                LineSegment lineSegment = new LineSegment(previousCoord, coord);
