...
public class ZCoordinateVertexMarkerDecorator
extends VertexMarkerDecorator {
    public static final String NAME = I18N.getString("es.kosmo.core.renderer.decorators.impl.ZCoordinateVertexMarkerDecorator.Vertex-Z-Coordinate");
    public static final Icon ICON = IconLoader.icon("ZValueDecorator.gif");

    public ZCoordinateVertexMarkerDecorator() {
        this(Color.BLACK, DEFAULT_FONT, "pixel");
    }

    public ZCoordinateVertexMarkerDecorator(Color color, Font font, String units) {
        this.name = NAME;
        this.icon = ICON;
        this.setColor(color);
        this.setFont(font);
    ...
    protected void paintGeometry(Geometry geometry, Graphics2D graphics, IDecoratorPoint2DTransformer viewport, double pixelSize, Unit<Length> viewUnit) throws Exception {
        ...
        while (i < coords.length) {
            ...
            String label = "";
            label = !Double.isNaN(currentCoord.z) ? "Z = " + NumberFormatManager.getFormattedValue(currentCoord.z) : "Z = NaN";
            this.setText(label);
