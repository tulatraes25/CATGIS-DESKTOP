...
public class PasteGeometryPlugIn
extends AbstractPlugIn {
    private static final Logger LOGGER = Logger.getLogger(PasteGeometryPlugIn.class);
    public static final String NAME = I18N.getString("es.kosmo.desktop.plugins.editing.PasteGeometryPlugIn.Paste-geometry-to-the-selected-feature");
    ...
    public boolean execute(PlugInContext context) throws Exception {
        ...
        boolean ok = PasteGeometryPlugIn.conformGeometryType(geom, editableLayer.getFeatureSchema().getGeometryType());
        if (!ok) {
        ...
        Feature selectedFeat = selectedFeatures.iterator().next();
        Feature cloneFeat = selectedFeat.clone(true);
        cloneFeat.setGeometry(geom);
        ...
        this.execute(new UndoableCommand(this.getName()){
            ...
            public void execute() throws Exception {
                try {
                    ...
                    editableLayer.getLayerManager().fireGeometryModified(featsToUpdate, editableLayer, selectedFeatures);
            ...
            public void unexecute() throws Exception {
                selectionManager.unselectItems(editableLayer);
                editableLayer.getFeatureCollectionWrapper().updateAll(selectedFeatures);
                editableLayer.getLayerManager().fireGeometryModified(selectedFeatures, editableLayer, featsToUpdate);
                selectionManager.getFeatureSelection().selectItems(editableLayer, selectedFeatures);
            }
        }, context);
    ...
    public EnableCheck getCheck() {
        return PasteGeometryPlugIn.createEnableCheck(JUMPWorkbench.getFrameInstance().getContext());
    }
