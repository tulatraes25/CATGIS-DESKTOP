...
import com.vividsolutions.jump.workbench.ui.snap.algorithms.PerpendicularSnapAlgs;
...
public class SnapToPerpendicularPolicy
implements SnapPolicy {
    private static final Logger LOGGER = Logger.getLogger(SnapToPerpendicularPolicy.class);
    private Color color;
    public static final String ENABLED_KEY = String.valueOf(SnapToPerpendicularPolicy.class.getName()) + " - ENABLED";
    public static final String NAME = I18N.getString("com.vividsolutions.jump.workbench.ui.snap.SnapToPerpendicularPolicy.Snap-to-perpendicular");
    private Blackboard blackboard;
    private static GeometryFactory factory = new GeometryFactory();
    private static PerpendicularSnapAlgs perpendicularAlgs = new PerpendicularSnapAlgs();

    public SnapToPerpendicularPolicy(Blackboard blackboard) {
    ...
    public Coordinate snap(LayerViewPanel panel, Coordinate originalPoint, Geometry toSpecificGeometry) {
        ...
        if (toSpecificGeometry != null) {
            vertices.addAll(perpendicularAlgs.extractPerpendicularPoints(originalPoint, lastClickedPoint, toSpecificGeometry, units));
        } else {
            for (Geometry pointsAndLines : VisiblePointsAndLinesCache.instance(panel).getTree().query(bufferedTransformedCursorLocation.getEnvelopeInternal())) {
                vertices.addAll(perpendicularAlgs.extractPerpendicularPoints(originalPoint, lastClickedPoint, pointsAndLines, units));
            }
        }
        if (vertices.isEmpty()) {
            return null;
